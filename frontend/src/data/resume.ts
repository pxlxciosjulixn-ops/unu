/**
 * La hoja de vida empaquetada con el sitio.
 *
 * La versión que manda es la de la base de datos (`GET /api/resume/`); esta se
 * usa sólo como respaldo: mientras llega la respuesta, si el backend está
 * dormido o si nadie ha guardado nada todavía, la página igual muestra un CV
 * completo en vez de una hoja en blanco.
 *
 * Tiene la misma forma que la respuesta de la API a propósito, para que los
 * componentes no tengan que saber de cuál de las dos vienen los datos.
 */
import type { HojaDeVida } from "@/lib/resume-api"

export const HOJA_DE_VIDA_POR_DEFECTO: HojaDeVida = {
  profile: {
  ...
  languages: [
    { name: "Español", level: "nativo", level_label: "Nativo", percent: 100 },
    {
      name: "Inglés",
      level: "intermedio",

...
/**
 * La misma tabla del dashboard pero con los botones de editar y borrar, y sin
 * filtro de periodo: aquí se busca una fila concreta para corregirla, y bien
  ...
  return (
    ...
    >
      <CardHeader>
        ...
        <CardDescription>
          {movimientos
            ? `${filtrados.length} ${filtrados.length === 1 ? "movimiento" : "movimientos"} en total`
            : "Cargando…"}
        </CardDescription>
        <CardAction>
          <ToggleGroup
            ...
            <TableBody>
              {filtrados.slice(0, visibles).map((m) => (
                <TableRow key={m.id}>
                  ...
                  {/* En el celular la fecha y el tipo van debajo del concepto:
                      así el valor no se sale de la pantalla. */}
      ...
      {movimientos && filtrados.length > visibles ? (
